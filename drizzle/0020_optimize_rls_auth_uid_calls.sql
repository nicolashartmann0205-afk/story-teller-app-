-- Optimize RLS policy expressions by evaluating auth.uid() once per statement.
-- Rewrites all public-schema policy expressions from auth.uid() to (select auth.uid()).
